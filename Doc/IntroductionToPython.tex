...
optimiser, obtaining an final training loss of 0.0424, and validation loss of
0.0464.
The ratio between the loss values is $\sim1.09$, this value is close to 1,
suggesting that the network was not over-fitting.

The network was trained again using the ``Adam'' optimiser, which uses
estimations of both first and second-order moments \citep{Kingma:2014:Adam}.
\begin{center}
    \import{../Code/}{BostonHousingTensorResults.pgf}
    \parbox{0.9\textwidth}{%
    \captionof{figure}{Graph of network error for training and validation
    against iteration number with ``Adam'' optimiser.}}
\end{center}
