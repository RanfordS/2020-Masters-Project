...
applies a function to a small region of the previous layer.
...
% network `invented' well known acoustic-phonetic features and useful abstractions
% detected three acoustically similar phonemes: /b/, /g/, and /d/ (voiced stops)
% can compare current inputs with past inputs
% sigmoid
% spectral coefficients as inputs
% convolves 3 time by 16 coefficients into 8 units
% convolves 5 time by 8 units into 3 units, each unit represents a phoneme
% sums each unit across time to and takes sigmoid to obtain likely phoneme
% learnt per-speaker
% performed better than Hidden Markov Model,
%   which had been the most popular method for phoneme recognition
%   average error reduced from 6.3\% to 1.5\%
