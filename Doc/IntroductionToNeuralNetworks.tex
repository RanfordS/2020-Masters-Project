...
% \cite{Jordan:1986:Serial}
% which describes how sequences of actions might be learned and performed
%   parallel interactions across time (coarticulation) and
%   parallel interactions across space (dual-task interference)
%   two aspects of a common underlying process
% natural interpretation as a dynamical system, and a continuous time
% perspective is possible
% critique of associative chaining
%   fails to allow different orderings of the same actions
% buffer critiques
%   problems accounting for error patterns
%   interactions between actions should depend on relative position, not absolute
% other critiques
%   control elements are different for different orderings of the same actions
%     ce for B in ABC differs from ce for B in CBA, requires large numbers of
%     units and performs badly
%   assume actions are produced in parallel
%     temporal ordering problems
%     difficulty with sequences in which there are repeated occurrences of actions
% distinction between state and output
% allows both temporal order and parallel influences to be represented in terms
% of activation
% if a network has one or more cycles, it is recurrent
%
% trained
...
He proposed an alternative architecture, where the input layer contains two
groups of units:
...
The state units are connected
\begin{center}
    \input{DiagramJordan.tex}
    \captionof{figure}{Example of Jordan's recurrent neural network.}
\end{center}
% each hidden layer unit takes all input and state units as its input
% each state unit takes all output units and all state units as its input
...
% \cite{Elman:1990:Finding}
% notes that linguists treat sentences as if available instantaneously
% represent time implicitly
% critiques of parallel
%   requires buffered input to present all data simultaneously
%   no evidence of biological shift registers
%   when should input be processed
%   suggests that all inputs should be the same length
%   cannot distinguish between absolute and relative temporal positions
% proposed systems with memory
%   ``allow time to be represented by the effect it has on processing''
\begin{center}
    \input{DiagramElman.tex}
    \captionof{figure}{Example of Elman's recurrent neural network.}
\end{center}
% each hidden unit has a corresponding state unit
% each state unit takes the corresponding hidden unit as its input
% each hidden unit takes the corresponding state unit and the previous layer as
% its input
