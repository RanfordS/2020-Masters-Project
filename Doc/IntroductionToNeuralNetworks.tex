...
Densely connected networks did not scale well for image recognition, as the
number of connections to each node for a high resolution image is too large.
...
% visual cortex contains neurons that individually respond to small
...
%\begin{align*}
%    y_{i,j} = \phi\left(b_{i,j} + \sum_{k=0}^s\sum_{l=0}^s w_{k,l}x_{i+k,j+l}\right)
%\end{align*}
