...
class Memory:
    ...
    def sample (self, num):
        ...
        b_weight = np.empty ((num, 1), dtype = np.float32)
        priority_segment = self.tree.total_priority/num
        max_weight = (self.tree.min_priority*num)**(self.b) #TODO: verify negative
        for i in range (num):
            a, b = priority_segment*i, priority_segment*(i + 1)
            value = np.random.uniform (a, b)
            index, priority, data = self.tree.get_leaf (value)
            sampling_probabilities = priority / self.tree.total_priority
            b_weight[i,0] = (num*sampling_probabilities)**(self.b) / max_weight
            b_index[i] = index
            batch.append (data)
...
for episode in range (num_episodes):
    ...
    for step in range (num_steps):
        ...
        for i in range (size_batch):
            ...
            b_target[i,b_action[i]] += lr*TD#(b_reward[i] + gamma*max (next_q[i]) - b_target[i,b_action[i]])
